...
test("panels retain semantic native controls", async () => {
  ...
  assert.match(styles, /\.reading-location-floater\s*\{[^}]*width:\s*fit-content[^}]*max-width:\s*calc\(100vw - var\(--edge-inset-left\) - var\(--edge-inset-right\)\)[^}]*grid-template-columns:\s*2\.5rem minmax\(0, max-content\) max-content 2\.5rem/);
